const TextCircle = ({ texts }: { texts: string }) =>{    
    ...
    return (<div className='ratate-text relative w-0 origin-center transform'>
            {textArray.map((text, index)=>(
                <span key={`${text}-${index}`} className={`text-lg h-[190px] md:text-4xl md:h-[330px] lg:h-[430px] text-yellow-200 absolute bottom-0 origin-[bottom_center] transform hover:bg-transparent`} style={{
